...
namespace RTC
{
    public partial class CWGAccount : Form
    {
        public CWGAccount()
        {
            InitializeComponent();
        ...
        private void populate()
        {
            try
            {
                ...
                con.Open();
                String Query = "select * from GarusaruWithdrawalsTB";
                SqlDataAdapter sda = new SqlDataAdapter(Query, con);
                SqlCommandBuilder builder = new SqlCommandBuilder(sda);
                var ds = new DataSet();
                sda.Fill(ds);
                GWDGV.DataSource = ds.Tables[0];
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void LoadAccountNumbers()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(conString))
                {
                    ...
                    AccountNoTB.DisplayMember = "GID"; // What the user sees
                    AccountNoTB.ValueMember = "GID";   // The actual value
            ...
            if (!string.IsNullOrEmpty(accountNo))
            {
                try
                {
                    using (SqlConnection conn = new SqlConnection(conString))
                    {
                        ...
                        string query = "SELECT Name, NIC, Balance FROM GarusaruAccountTB WHERE GID = @AccountNo";
        ...
        private void WithdrawBt_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(conString))
            {
                ...
                {
                    // Update Balance
                    string updateQuery = "UPDATE  GarusaruAccountTB SET Balance = Balance - @WithdrawAmount WHERE GID = @AccountNo";
                    ...
                    // Log Withdrawal
                    string insertQuery = "INSERT INTO GarusaruWithdrawalsTB (GID, Date, Withdraw_amount) VALUES (@AccountNo, @Date, @WithdrawAmount)";
        ...
        {
            string balanceQuery = "SELECT Balance FROM GarusaruAccountTB WHERE GID = @AccountNo";
