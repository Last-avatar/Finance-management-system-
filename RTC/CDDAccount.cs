...
namespace RTC
{
    public partial class CDDAccount : Form
    {
        public CDDAccount()
        {
            InitializeComponent();
        ...
        private void populate()
        {
            try
            {
                ...
                con.Open();
                String Query = "select * from DivisaruAccountCashTB";
        ...
        private void GetCategories()
        {
            SqlConnection con = new SqlConnection(conString);
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT DID FROM DivisaruAccountTB", con);
            SqlDataReader rdr = cmd.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Columns.Add("DID", typeof(int));
            dt.Load(rdr);

            AccountNoTb.ValueMember = "DID"; // Ensure this matches the column in the database
            AccountNoTb.DisplayMember = "DID"; // Optional, can set to another column like account name
        ...
        private void DepositBt_Click(object sender, EventArgs e)
        {
                ...
                using (SqlConnection connection = new SqlConnection(conString))
                {
                    try
                    {
                        ...
                        string checkAccountQuery = "SELECT ISNULL(Balance, 0) AS Balance FROM DivisaruAccountTB WHERE DID = @accountNo";
                        ...
                        string updateBalanceQuery = "UPDATE DivisaruAccountTB SET Balance = ISNULL(Balance, 0) + @depositAmount WHERE DID = @accountNo";
                        ...
                        SqlCommand getUpdatedBalanceCommand = new SqlCommand(checkAccountQuery, connection);
                        getUpdatedBalanceCommand.Parameters.AddWithValue("@accountNo", accountNo);
                        result = getUpdatedBalanceCommand.ExecuteScalar();
                        double newBalance = Convert.ToDouble(result);

                        // Log the deposit in DivisaruAccountCashTB
                        string insertDepositQuery = "INSERT INTO DivisaruAccountCashTB (DID, Name, NIC, Deposit, Date) " +
        ...
        int key = 0;
        private void CDDGV_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            AccountNoTb.Text = CDDGV.SelectedRows[0].Cells[1].Value.ToString();
            NameTB.Text = CDDGV.SelectedRows[0].Cells[2].Value.ToString();
            NICTb.Text = CDDGV.SelectedRows[0].Cells[3].Value.ToString();
            dateTB.Text = CDDGV.SelectedRows[0].Cells[4].Value.ToString();
            DepositTB.Text = CDDGV.SelectedRows[0].Cells[5].Value.ToString();
            ...
            {
                key = Convert.ToInt32(CDDGV.SelectedRows[0].Cells[0].Value.ToString());
            }
        }
    }
}
