...
namespace RTC
{
    public partial class DCollections : Form
    {
        public DCollections()
        {
            InitializeComponent();
        ...
        private void populate()
        {
            try
            {
                ...
                con.Open();
                String Query = "select * from DivisaruCollectionsTB";
                SqlDataAdapter sda = new SqlDataAdapter(Query, con);
                SqlCommandBuilder builder = new SqlCommandBuilder(sda);
                var ds = new DataSet();
                sda.Fill(ds);
                DCDGV.DataSource = ds.Tables[0];
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void InsertDoument()
        {
            ...
            else
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("INSERT INTO DivisaruCollectionsTB (DID,Name,Amount,Date) VALUES (@DID,@DN,@DA,@DD)", con);

                    // Add parameters
                    cmd.Parameters.AddWithValue("@DID", accountNo); // Use SelectedValue for LID
                    cmd.Parameters.AddWithValue("@DN", NameTB.Text);
                    cmd.Parameters.AddWithValue("@DA", AmountTB.Text);
        ...
        {
            string query = "SELECT  Name FROM DivisaruAccountTB WHERE DID = @AccountNo AND Status = 'Approve'";
        ...
        private void DeleteDocument()
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    string query = "DELETE FROM DivisaruCollectionsTB WHERE CID = @Key"; // Replace 'ID' with your actual column name
        ...
        private void DCDGV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = DCDGV.Rows[e.RowIndex];
